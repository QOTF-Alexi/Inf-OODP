...
{
    private List<SpellTypes> _knownSpells = new();
    ...
    public Wizard(string name) : this(name, new List<SpellTypes>() ) { }
    public Wizard(string name, List<SpellTypes> spells) :
        base(name, 80, 3, 3)
    {
        _knownSpells = new() { SpellTypes.FireBolt, SpellTypes.Heal };
        _knownSpells.AddRange(spells);
    ...
    {
        CastSpell(spellType, new ITargetable[] { target });
    }
