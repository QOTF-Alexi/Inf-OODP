public class Program
{
    static void Main()
    {
        ...
        var correctedGrades1 = grades1.Select(n => n + 1);
        ...
        var correctedGrades2 = grades2.Select(n => Math.Min(n + 1, 10));
        Console.WriteLine(string.Join(", ", correctedGrades2));

        // Only keep the grades that are 6 or higher
        var passingGrades = grades2.Where(n => n >= 6);
        ...
        var studentsSortedByAge = students.OrderBy(student => student.Item2);
        Console.WriteLine(string.Join(", ", studentsSortedByAge));

        // Get the highest grade of two chances
        int gradeFirstChance = 5;
        int gradeRetake = 8;
        Func<int, int, int> higherGrade = (grade1, grade2) => Math.Max(grade1, grade2);;
