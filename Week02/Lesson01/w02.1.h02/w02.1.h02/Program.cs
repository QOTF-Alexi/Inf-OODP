...
{
 Console.WriteLine("Where would you like to go? (N/E/S/W)");
 string goToDir = Console.ReadLine().ToUpper();
 if (goToDir is "N" or "E" or "S" or "W")
 {
  player.TryMoveTo(player.CurrentLocation.GetLocationAt(goToDir));
 }
}
// ^^^YOUR CODE GOES HERE ^^^
